(function(){
  ...
  const CONFIG = {
    ...
    MENU_PRICE_STEP: 0.1,
    MENU_PRICE_MAX_PER_MICHELIN: 0.15,
    FAVORITE_MATCH_BONUS: 0.25,
    GEM_SPEEDUP_COST: 5,
    CRITIC_EXCLUSIVE_REP: 80,
  ...
  function menuPriceMax(){
    return (CONFIG.MENU_PRICE_MAX || 1.8) + (state.michelinStars || 0) * (CONFIG.MENU_PRICE_MAX_PER_MICHELIN || 0.15);
  }
  function menuPriceMult(){
    const p = state.menuPrice == null ? (CONFIG.MENU_PRICE_DEFAULT || 1) : state.menuPrice;
    return Math.max(CONFIG.MENU_PRICE_MIN || 0.6, Math.min(menuPriceMax(), p));
  }
  ...
  function fulfillOrder(){
    ...
    let favBonus = 1;
    if(def.favorite && state.activeRecipe && state.activeRecipe.endsAt > Date.now()){
      const style = def.favorite;
      if(style === 'signature' || style === 'classic' || style === 'quick' || style === 'large'){
        favBonus = 1 + (CONFIG.FAVORITE_MATCH_BONUS || 0.25);
      }
    }
    const gain = (rate * CONFIG.ORDER_REWARD_SECONDS + nextTapGain() * CONFIG.ORDER_REWARD_TAP_MULT)
      * budget * loyaltyRewardMult() * satisfactionMult() * favBonus;
    ...
    // Critic exclusive opportunity (Part 5.3)
    if(def.special === 'critic' && (state.reputation || 0) >= (CONFIG.CRITIC_EXCLUSIVE_REP || 80) && remainFrac > 0.4){
      state.researchPoints = (state.researchPoints || 0) + 2;
      earnDiamonds(1);
      if(typeof pushSocialFeed === 'function') pushSocialFeed('Critic exclusive: +2 Research & a gem!');
    }
  ...
  function renderEconomy(){
    ...
    html += `<div class="rep-panel-card">
      ...
      <input type="range" id="menuPriceSlider" min="${CONFIG.MENU_PRICE_MIN}" max="${menuPriceMax()}" step="${CONFIG.MENU_PRICE_STEP}" value="${price}"
    ...
    country.businesses.forEach((def, idx) => {
      ...
      card.innerHTML = `
        <div class="biz-main" data-action="toggle" data-id="${def.id}" ${toggleAttrs}>
          ...
          ${b.level>0?`<button class="buy-btn secondary" data-action="gem-speed" data-id="${def.id}" title="Spend ${CONFIG.GEM_SPEEDUP_COST||5} 💎 for +1 level" style="min-width:44px;">💎${CONFIG.GEM_SPEEDUP_COST||5}</button>`:''}
        </div>
  ...
  function claimSeasonal(){
    ...
    // Holiday exclusive chef fragment (Part 8.3)
    if(typeof addChefFragment === 'function' && typeof LEGENDARY_CHEFS !== 'undefined'){
      const map = { halloween:'diego', holiday:'nonna', newyear:'emperor', valentine:'priya', spring:'ichiro', summer:'diego' };
      const chefId = map[ev.id];
      if(chefId) addChefFragment(chefId, 3);
    }
    save();
    renderAchievements();
    renderStats();
  ...
  function finishChampionship(){
    ...
    if(place === 1){
      ...
      earnDiamonds(5);
      adjustFame(20);
      if(typeof addChefFragment === 'function'){
        const chefs = (typeof LEGENDARY_CHEFS !== 'undefined' ? LEGENDARY_CHEFS : []).map(c => c.id);
        if(chefs.length) addChefFragment(chefs[Math.floor(Math.random()*chefs.length)], 2);
      }
    } else if(place === 2){
      earnEventTokens(12);
      earnDiamonds(2);
      if(typeof addChefFragment === 'function'){
        const chefs = (typeof LEGENDARY_CHEFS !== 'undefined' ? LEGENDARY_CHEFS : []).map(c => c.id);
        if(chefs.length) addChefFragment(chefs[Math.floor(Math.random()*chefs.length)], 1);
      }
    } else if(place === 3){
      earnEventTokens(8);
      earnDiamonds(1);
  ...
  function renderEvents(){
    ...
    EVENT_DECORATIONS.forEach(d => {
      ...
      html += `<div class="ach-card${owned?' claimed':''}" style="margin-bottom:6px;">
        <div class="ach-icon${owned?' done animated-deco':''}">${d.icon}</div>
  ...
  // Gem speed-up: spend diamonds for +1 shop level without cash (Part 6 gap)
  function gemSpeedupBusiness(id){
    const cost = CONFIG.GEM_SPEEDUP_COST || 5;
    if((state.diamonds || 0) < cost){ playErrorSfx(); return; }
    const country = activeCountryDef();
    const def = country.businesses.find(d => d.id === id);
    if(!def) return;
  ...
  bizPanel.addEventListener('click', e => {
    ...
    else if(action === 'gem-speed') gemSpeedupBusiness(id);
